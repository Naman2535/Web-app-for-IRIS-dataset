...
st.write("Predicted flower *Species* is ",
         str(iris.target_names[prediction]))
...
st.write(
    ...
    """.format(prediction_proba[0][0] * 100, prediction_proba[0][1] * 100, prediction_proba[0][2] * 100)
)
...
if st.checkbox("Show EDA?"):
    ...
    sns.heatmap(data.corr(), annot=True, vmin=-1, vmax=1, center=0, cmap='Blues', fmt='.2f', mask=matrix,
                linewidths=.5, square=True, linecolor='white', annot_kws={'size': 10}, cbar=True)
    st.pyplot()

    fig, axes = plt.subplots(2, 2, figsize=(16, 9))

    sns.FacetGrid(data, hue="species", height=5).map(
        sns.distplot, "petal_width", ax=axes[0, 0]).add_legend()
    st.pyplot()

    sns.FacetGrid(data, hue="species", height=5).map(
        sns.distplot, "petal_length", ax=axes[0, 1]).add_legend()
    st.pyplot()

    sns.FacetGrid(data, hue="species", height=5).map(
        sns.distplot, "sepal_length", ax=axes[1, 0]).add_legend()
    st.pyplot()

    sns.FacetGrid(data, hue="species", height=5).map(
        sns.distplot, "sepal_width", ax=axes[1, 1]).add_legend()
    st.pyplot()

    fig, axes = plt.subplots(2, 2, figsize=(16, 9))
    sns.violinplot(y="petal_width", x="species",
                   data=data,  orient='v', ax=axes[0, 0])
    sns.violinplot(y="petal_length", x="species",
                   data=data,  orient='v', ax=axes[0, 1])
    sns.violinplot(y="sepal_length", x="species",
